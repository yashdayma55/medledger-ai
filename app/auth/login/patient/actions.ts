...
const errorRedirect = (message: string): never =>
  redirect(`/auth/login/patient?error=${encodeURIComponent(message)}`);

export async function loginPatient(formData: FormData): Promise<never> {
  ...
  if (role !== "patient") {
    if (role === "provider") {
      errorRedirect("This email is registered as a doctor. Please use the doctor login page.");
    }
    errorRedirect("Invalid email or password.");
  }
  if (status === "disabled") {
